  ...
  return (
    <div className={cn('inline-block', className)}>
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-2 rounded-lg border border-border bg-white py-1.5 pl-3 pr-2 text-xs font-semibold capitalize text-heading outline-none transition hover:border-navy-300 focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-glow"
      >
        <span>{fmt(value)}</span>
                ...
                {options.map((o) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-left text-xs font-medium capitalize transition',
    ...
    return (
      <span className="inline-flex items-center gap-1">
        <button
          ...
          className="inline-flex items-center gap-1 rounded-lg bg-error px-2 py-1 text-xs font-semibold text-white transition hover:opacity-90"
        >
          <Check className="h-3.5 w-3.5" /> Confirm
        </button>
        <button
          onClick={() => setArmed(false)}
          className="rounded-lg border border-border bg-surface px-2 py-1 text-xs font-semibold text-body transition hover:bg-surface-alt"
        >
          Cancel
        </button>
      </span>
...
export function PageHeader({
  ...
  return (
    <div className="relative mb-8">
      {/* Fading grid behind the title */}
      <div
        aria-hidden
        className="pointer-events-none absolute -left-10 -top-14 -z-10 h-72 w-[min(60rem,100%)]"
        style={{
          backgroundImage:
            'linear-gradient(var(--color-navy-600) 1px, transparent 1px), linear-gradient(90deg, var(--color-navy-600) 1px, transparent 1px)',
          backgroundSize: '56px 56px',
          opacity: 0.12,
          WebkitMaskImage: 'radial-gradient(120% 110% at 0% 0%, #000 14%, transparent 74%)',
          maskImage: 'radial-gradient(120% 110% at 0% 0%, #000 14%, transparent 74%)',
        }}
      />
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-[2.5rem] font-extrabold leading-[1.05] tracking-tight text-heading sm:text-[3rem]">
            {title}
          </h1>
...
export function Th({ children, className }: { children?: React.ReactNode; className?: string }) {
  return (
    <th
      className={cn(
        'whitespace-nowrap border-b border-navy-800 bg-navy-600 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-navy-100',
        className,
      )}
    >
      {children}
...
export function Td({ children, className }: { children?: React.ReactNode; className?: string }) {
  return (
    <td className={cn('whitespace-nowrap border-b border-border-light px-4 py-3 text-body', className)}>
      {children}
    </td>
  );
}
