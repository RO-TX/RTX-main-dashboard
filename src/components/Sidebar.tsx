...
export function Sidebar() {
  ...
  const renderItem = (item: NavItem) => {
    ...
    return (
      <Link
        ...
        className={cn(
          'group relative flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-base font-medium transition-colors duration-200',
          collapsed && 'lg:justify-center lg:px-0',
          active ? 'text-white' : 'text-navy-200 hover:bg-white/10 hover:text-white',
        )}
      >
        {active && (
          <motion.span
  ...
  const groupLabel = (text: string) => (
    <p
      className={cn(
        'mb-1 mt-5 px-3 text-xs font-semibold uppercase tracking-wider text-navy-400',
        collapsed && 'lg:hidden',
      )}
    >
      {text}
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-40 flex h-screen w-64 shrink-0 flex-col overflow-hidden border-r border-white/10 bg-navy-900 px-4 py-6 transition-[transform,width] lg:static lg:translate-x-0',
      ...
      >
        ...
        <div className={cn('relative z-10 flex items-center gap-2 px-2', collapsed && 'lg:justify-center lg:px-0')}>
          ...
          <div className={cn('leading-tight', collapsed && 'lg:hidden')}>
            <p className="text-base font-extrabold text-white">RO TECHNICAL</p>
            <p className="text-xs tracking-[0.3em] text-navy-300">XPERTS</p>
          </div>
          <button
        ...
        <div className="relative z-10 mt-4 border-t border-white/10 pt-4">
          <div className={cn('flex items-center gap-3 px-2', collapsed && 'lg:justify-center lg:px-0')}>
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-accent text-base font-bold text-white">
              {user?.firstName?.[0] ?? 'A'}
            </div>
            <div className={cn('min-w-0 flex-1 leading-tight', collapsed && 'lg:hidden')}>
              ...
              <p className="truncate text-sm capitalize text-navy-300">
                {user?.role?.replace('_', ' ')}
              </p>
            </div>
          </div>
          <button
            ...
            className={cn(
              'mt-3 flex w-full items-center gap-3 rounded-xl px-3.5 py-2.5 text-base font-medium text-navy-200 transition hover:bg-error/20 hover:text-white',
              collapsed && 'lg:justify-center lg:px-0',
            )}
          >
